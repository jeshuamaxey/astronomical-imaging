% set initial values
done=0;                         % used in while loop
threshold=2000;                 %
lbg_threshold=4000;             % WTF IS THIS VALUE?
r=0;                            % half aperture size (in pixels)
non_linear_threshold = 40000;   % the pixel count beyond which the CCD becomes non-linear

%main
loadimage();
maskperimeter();
maskabovenonelinearthreshold();

% while loop to mask all pixels above local background threshold
while ~done
...
fitswrite(img_masked,'localmasked.fits');
